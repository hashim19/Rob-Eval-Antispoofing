...
if 'dev'==args[1]:
    ...
    CM_SCOREFILE = 'RawGAT_ST_add_LA_eval_CM_scores.txt'
...
if cm_data.shape[1] >= 3:  # Making sure there are at least three columns
    cm_utt_id = cm_data[:, 0]
    cm_scores = cm_data[:, 3].astype(float)  
    cm_keys = cm_data[:, 2] 
else:
    print("Error: Expected at least three columns in the CM data.")
    sys.exit()  # Or handle the error as needed
